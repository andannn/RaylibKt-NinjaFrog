...
class EnemyEntity(override val state: Positional2D) : Positional2DEntity {
...
fun ComponentRegistry.enemy() = component("enemy") {
    ...
    enemyAi(enemyEntity)
    positional2DEntityComponent(enemyEntity) {
        mainCharacterSpritAnimation(
            MainCharacter.NINJA_FROG,
            SIZE,
            SIZE,
            enemyEntity.spriteAnimationState
        )
    }
}
...
private fun ComponentRegistry.enemyAi(
    ...
    onUpdate { dt ->
        enemyEntity.checkXAxisCollision(
            dt,
            speedVector,
        )

        enemyEntity.checkYAxisCollision(
            dt,
            speedVector
        )

        enemyEntity.updatePositionBySpeed(dt, speedVector)
    }
}
